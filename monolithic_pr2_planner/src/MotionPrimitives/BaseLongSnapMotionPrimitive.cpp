...
bool BaseLongSnapMotionPrimitive::apply(const GraphState& source_state, 
    ...
    { 
      ...
      // ContBaseState modified_base;;
      // modified_base.x(source_state.robot_pose().getContBaseState().x());
      // modified_base.y(source_state.robot_pose().getContBaseState().y());
      // modified_base.z( m_goal->getRobotState().getContBaseState().z());
