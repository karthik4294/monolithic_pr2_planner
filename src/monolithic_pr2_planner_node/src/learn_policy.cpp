#include <monolithic_pr2_planner_node/learn_policy.h>
...
LearnPolicy::LearnPolicy(ros::NodeHandle nh) : m_env(new Environment(nh, true)), m_env_interface(m_env,
  nh){
    m_env_interface.bindPlanRandomPathsToEnv("/sbpl_planning/plan_path");
...
int main(int argc, char** argv){
    ros::init(argc, argv, "monolithic_pr2_planner_learn_node");
    ros::NodeHandle nh("~");
    setLoggersFromParamServer(nh);
    LearnPolicy learn_node(nh);
